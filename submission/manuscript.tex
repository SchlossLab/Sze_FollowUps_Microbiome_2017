...
probabilities increased (358 (121) days) or decreased (334 (143) days)
(Wilcoxon Test; P-value = 0.56) (all days data displayed as median
(IQR)). Finally, it is possible that these patients may not have
...
\textbf{\emph{Model Building.}} {[}Comment: This entire section is
pretty confusing. In the first paragraph it jumbles the treatment models
and the diagnosis models{]} The Random Forest {[}33{]} algorithm was
...
The model included only OTU data obtained from 16S rRNA sequencing and
was constructed using the R package caret (v6.0.73). Optimization of the
mtry hyper-parameter involved making 100 different 80/20 (train/test)
splits of the data where normal and adenoma, normal and advanced
adenoma, or normal and carcinoma were represented in the same proportion
within both the whole data set and the 80/20 split. For each of the
different splits, 20 repeated 10-fold cross validation was performed on
the 80\% component to optimize the mtry hyper-parameter by maximizing
...
the 20\% component. All three models had an optimized mtry of 2. The
hyper-parameter, mtry, defines the number of variables to investigate at
each split before a new division of the data was created with the Random
Forest model.
