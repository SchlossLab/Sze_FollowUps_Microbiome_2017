...
%DIF DEL submission/manuscript_R1.tex   Fri Sep 15 13:28:54 2017
%DIF ADD submission/manuscript_R2.tex   Sat Sep 16 13:34:02 2017
\usepackage{lmodern}
...
groups. Although the \DIFdelbegin \DIFdel{three models could }\DIFdelend \DIFaddbegin \DIFadd{adenoma and carcinoma models could reliably
}\DIFaddend differentiate between the pre and post-treatment samples \DIFaddbegin \DIFadd{(P-value
\textless{} 0.001), the advanced-adenoma model could not (P-value =
0.61). Furthermore}\DIFaddend , there was little overlap between the OTUs that were
indicative of each treatment. To determine whether individuals who
underwent treatment were more likely to have OTUs associated with normal
colons we used a larger cohort that contained individuals with normal
colons and those with adenomas, advanced adenomas, and carcinomas. We
again built Random Forest models and measured the change in the positive
probability of having one of the three diagnoses to assess whether the
post-treatment samples received the same classification as the
pre-treatment samples. Samples from patients who had carcinomas changed
towards a microbial milieu that resembles the normal colon after
treatment (P-value \textless{} \DIFdelbegin \DIFdel{0.05}\DIFdelend \DIFaddbegin \DIFadd{0.001}\DIFaddend ). Finally, we were unable to detect
any significant differences in the microbiota of individuals treated
with surgery alone and those treated with chemotherapy or chemotherapy
and radiation (P-value \textgreater{} 0.05).
...
lesion would \DIFdelbegin \DIFdel{affect the microbiota and }\DIFdelend \DIFaddbegin \DIFadd{not only remove the lesion, but also the microbiota that
promoted the tumorigenesis and hence the }\DIFaddend risk of recurrence. To test
...
OTUs. Despite this restriction, the }\DIFdelend \DIFaddbegin \DIFadd{The adenoma and carcinoma }\DIFaddend models performed
well (adenoma AUC range = \DIFdelbegin \DIFdel{0.69 }\DIFdelend \DIFaddbegin \DIFadd{0.54 }\DIFaddend - \DIFdelbegin \DIFdel{0.92, advanced adenoma }\DIFdelend \DIFaddbegin \DIFadd{0.83 and carcinoma }\DIFaddend AUC range = \DIFdelbegin \DIFdel{0.80 }\DIFdelend \DIFaddbegin \DIFadd{0.82 }\DIFaddend -
\DIFdelbegin \DIFdel{1.00, carcinoma
}\DIFdelend \DIFaddbegin \DIFadd{0.98); however, the model for patients treated for advanced adenomas was
not able to reliably differentiate between the pre and post-treatment
samples (advanced adenoma }\DIFaddend AUC range = \DIFdelbegin \DIFdel{0.82 }\DIFdelend \DIFaddbegin \DIFadd{0.34 }\DIFaddend - \DIFdelbegin \DIFdel{0.98}\DIFdelend \DIFaddbegin \DIFadd{0.65}\DIFaddend ). Interestingly, the
\DIFaddbegin \DIFadd{top }\DIFaddend 10 \DIFdelbegin \DIFdel{OTUs }\DIFdelend \DIFaddbegin \DIFadd{most important OTUs by MDA }\DIFaddend that were used for each model had
...
not consistent across patients and diagnosis groups. \DIFaddbegin \DIFadd{Both the adenoma
...
Helsinki Declaration. \DIFaddbegin \DIFadd{The original protocol for the study did not
provide for tracking patients after the follow up samples and so it was
not possible for us to ascertain their diagnosis after the completion of
the study.
}\DIFaddend 
...
versus post-treatment carcinoma model }%DIFDELCMD < {[}%%%
\DIFdel{Table 1}%DIFDELCMD < {]}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Similarly, the Random Forest }%DIFDELCMD < {[}%%%
\DIFdel{33}%DIFDELCMD < {]} %%%
\DIFdel{algorithm was also used to create
the
...
normal versus diagnosis models was 423 individuals }%DIFDELCMD < {[}%%%
\DIFdel{Table 2}%DIFDELCMD < {]}%%%
\DIFdel{. There
...
individuals in the normal versus carcinoma model }%DIFDELCMD < {[}%%%
\DIFdel{Table 2}%DIFDELCMD < {]}%%%
\DIFdel{. }%DIFDELCMD < 
...
patient's original diagnosis or that of a disease-free patient. }\DIFaddend All
models included only OTU data obtained from 16S rRNA sequencing and were
processed \DIFdelbegin \DIFdel{and cleaned using the R package }\DIFdelend \DIFaddbegin \DIFadd{using the }\DIFaddend caret (v6.0.76) \DIFdelbegin \DIFdel{.
Optimization of }\DIFdelend \DIFaddbegin \DIFadd{R package. For each model we
optimized }\DIFaddend the mtry hyper-parameter\DIFdelbegin \DIFdel{involved making }\DIFdelend \DIFaddbegin \DIFadd{, which defines the number of OTUs to
...
not result in over-fitting of the data, we made }\DIFaddend 100 different 80/20
(train/test) splits of the data where the same proportion was present
within both the whole data set and the 80/20 split. For each of the \DIFdelbegin \DIFdel{different }\DIFdelend \DIFaddbegin \DIFadd{100
}\DIFaddend splits, 20 repeated 10-fold cross validation was performed on the 80\%
...
resulting model was then tested on the \DIFdelbegin \DIFdel{hold out data obtained from
the }\DIFdelend 20\% \DIFdelbegin \DIFdel{component. For all pre versus post-treatment models the
...
variables to investigate at each split before a new division }\DIFdelend of the data \DIFdelbegin \DIFdel{was created with the Random Forest model }%DIFDELCMD < {[}%%%
\DIFdel{33}%DIFDELCMD < {]}%%%
\DIFdel{. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{For each of the pre versus post-treatment models assessment of the most
...
OTU pre versus post-treatment model by diagnosis group to
help avoid
...
\DIFdel{of MDA for each }\DIFdelend \DIFaddbegin \DIFadd{that were held
...
a random labeling was assessed by comparing the distribution }\DIFaddend of the 100
\DIFdelbegin \DIFdel{different splits . This was then followed with filtering of this list to variables that were only present in more than 50}%DIFDELCMD < \% %%%
\DIFdel{of
these 100 runs.
These corresponding reduced OTU normal versus }\DIFdelend \DIFaddbegin \DIFadd{80/20 splits for the correctly labeled data to the distribution of
randomly labeled data.
}

\DIFadd{The three }\DIFaddend diagnosis models were \DIFdelbegin \DIFdel{then
...
%DIFDELCMD < %%%
\DIFdel{Each model was then applied to our 67-person cohort }%DIFDELCMD < {[}%%%
\DIFdel{Table 1}%DIFDELCMD < {]} %%%
\DIFdel{based
...
(}\DIFdelend \DIFaddbegin \DIFadd{constructed by using the data from
Baxter et al. (24), which was censored for the }\DIFaddend pre-treatment \DIFdelbegin \DIFdel{carcinoma (carcinoma n = 26 and disease free n = 0)
versus post-treatment carcinoma (carcinoma n = 1 and disease free n =
25)). The application of
the pre versus }\DIFdelend \DIFaddbegin \DIFadd{samples of
the patients that we had }\DIFaddend post-treatment \DIFdelbegin \DIFdel{models generated
the probabilities that the sample was a pre-treatment sample. The application of the normal versus diagnosis models
generated the probabilities that the sample was that specific diagnosis (adenoma,
advanced adenoma, or carcinoma)}\DIFdelend \DIFaddbegin \DIFadd{samples. The treatment models
were then used to quantify the model probability that a patient with an
initial diagnosis retained that diagnosis or a disease-free diagnosis}\DIFaddend .
