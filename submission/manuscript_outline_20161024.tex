...
used at a single point in time to predict disease. In this current study
we attempt to combine specific physical characteristics, patient
history, Fecal Immunochemical Test (FIT), and the bacterial microbiome
to build the best model for classifying those with and without lesion
(adenoma or carcinoma). This model was tested on a \texttt{66} person
group that included samples before and after treatment to allow for the
assessment of how the model adjusts risk after treatment. Using NMDS,
the thetayc distance between initial and follow up showed a significant
difference (P-value = \texttt{0.001}) for the carcinoma group only. The
model chosen had a 10-fold cross validated AUC of \texttt{0.819} and a
test set AUC of \texttt{0.815}. There was a significant decrease in
probabliity of lesion from the model after treatment for both the
adenoma (P-value = \texttt{8.48e-07}) and carcinoma (P-value =
\texttt{5.96e-08}) group. The top 5 most important operational taxonomic
units for this model were \texttt{Lachnospiraceae\ (Otu000013)},
\texttt{Escherichia/Shigella\ (Otu000018)},
\texttt{Ruminococcaceae\ (Otu000020)},
\texttt{Ruminococcus\ (Otu000017)}, and
\texttt{Porphyromonas\ (Otu000153)}. For prediction on samples with
follow up the initial sensitivity was \texttt{98.5}\% (\texttt{65/66})
and follow up sensitivity was \texttt{100}\% (\texttt{1\ /\ 1}).
Overall, this model can accurately predict and subsequently respond to
treatment by lowering the probability of lesion.
