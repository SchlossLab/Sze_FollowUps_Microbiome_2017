...
changes between pre (initial) and post (follow up) samples after surgery
for individuals with a lesion (adenoma or carcinoma). Our analysis
...
OTUs within them, performed pre and post surgery. We also investigated
the models for similar important OTUs to identify the crucial OTUs for
not only classifying initial and follow up samples but also lesion or
normal.
...
towards what wouldbe be expected for normal controls is not known.
...
sample models initial positive probabilites were tested against the
...
Relevent Neoplasia (SRN). D) Initial positive probability change from
