...
test_cohort <- good_metaf %>% select(Dx_Bin, dx, Age, Gender, BMI, White, time)


t2 <- as.data.frame(matrix(nrow = 6, ncol = 3, 
                           dimnames = 
...
ft1 <- cbind(Group = c("Adenoma", "Advanced Adenoma", "Carcinoma"), ttemptable$Dx_Bin, ttemptable$Age, ttemptable$Gender_f, 
             ttemptable$BMI, ttemptable$White, ttemptable$post_time)


age_label <- paste("Age (Mean ", label = "\u00B1", " SD)", sep = "")
bmi_label <- paste("BMI (Mean ", label = "\u00B1", " SD)", sep = "")
post_time_label <- paste("Post-Treatment Sample (Mean ", label = "\u00B1", " SD)", sep = "")
