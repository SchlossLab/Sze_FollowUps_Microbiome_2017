...
Despite growing evidence that members of the gut microbiota can drive tumorigenesis, little is known about how treatment impacts the colonic community. This study tested the hypothesis that treatment for adenoma or carcinoma alters the abundance of bacterial populations associated with disease to those associated with a normal colon. We tested this hypothesis by sequencing the 16S rRNA genes in the feces of `r length(rownames(good_metaf))` individuals before and after treatment for adenoma (N = `r change_summary["thetayc", "adn_n"]`), advanced adenoma (N = `r change_summary["thetayc", "srn_n"]`), and carcinoma (N = `r change_summary["thetayc", "crc_n"]`). The structure of the microbiota in all three groups changed following treatment (P`r ifelse(change_summary["thetayc", "Pvalue_adnVcrc"]<0.001,"<0.001", format(change_summary["thetayc", "Pvalue_adnVcrc"], scientific = FALSE, digits = 2, nsmall = 2))`). To determine whether individuals who underwent treatment were more likely to have OTUs associated with normal colons we used a larger cohort that contained individuals with normal colons (N=XXX) and untreated individuals with adenomas (N=XXX), advanced adenomas (N=XXX), and carcinomas (N=XXX). Using these data, we built Random Forest models and measured the change in the positive probability of having one of the three diagnoses to assess whether the post-treatment samples received the same classification as the pre-treatment samples. The microbiota of patients diagnosed with carcinomas changed towards that of a normal colon following treatment (P<0.XXX). Those were initially diagnosed as having adenomas or advanced adenomas were no more likely to have a microbiota resembling that of a normal colon. These results further illuminate our understanding of the role of the gut microbiota in tumorigenesis and demonstrate the potential for using the gut microbiota as biomarkers for assessing risk and the effectiveness of treatment.
...
Colorectal cancer is a worldwide health problem. Previous work in animal models and human populations has demonstrated that the gut microbiota has an important role in promoting tumorigenesis through a variety of mechanisms. Less well understood is how the gut microbiota changes following treatment for varying stages of tumorigenesis. We observed that following treatment of patients diagnosed with carcinoma, their gut microbiota is likely revert to a state associated with individuals who had normal colons whereas those diagnosed with adenomas and advanced adenomas were not. This study was a necessary step to ultimately investigating how the microbiota changes in those with carcinoma recurrence and predicting which patients are more likely to recur. Finding better strategies to stratify patients based on recurrence risk could provide a more effective means of planning therapies and monitoring patients after treatment.
