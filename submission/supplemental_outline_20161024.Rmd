...
**Table S2: Summary of OTUs in Adenoma, Advanced Adenoma, and Carcinoma Models**
...
adn_varsMDA <- read.csv('../data/process/tables/adn_reduced_model_top_vars_MDA_Summary.csv', header = T, stringsAsFactors = F)
...
# Create MDA vectors
adn_MDA <- filter(adn_varsMDA, otu %in% common_vars) %>% slice(match(common_vars, otu)) %>% select(median_MDA)
srn_MDA <- filter(srn_varsMDA, otu %in% common_vars) %>% slice(match(common_vars, otu)) %>% select(median_MDA)
crc_MDA <- filter(crc_varsMDA, otu %in% common_vars) %>% slice(match(common_vars, otu)) %>% select(median_MDA)

st2 <- select(common_vars_summary, otu) %>% 
  mutate(Tax = gsub("_unclassified", "", common_vars_summary$lowest_ID)) %>% 
  mutate(adn_MDA = adn_MDA$median_MDA, srn_MDA = srn_MDA$median_MDA, crc_MDA = crc_MDA$median_MDA) %>% 
