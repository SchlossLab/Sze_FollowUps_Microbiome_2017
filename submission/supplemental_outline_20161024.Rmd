...
**Table S2: Summary of OTUs in Adenoma, SRN, and Carcinoma Models**
...
adn_varsMDA <- read.csv('../data/process/tables/adn_reduced_crc_model_top_vars_MDA_Summary.csv', header = T, stringsAsFactors = F)

srn_varsMDA <- read.csv('../data/process/tables/srn_reduced_model_top_vars_MDA_Summary.csv', header = T, stringsAsFactors = F)

crc_varsMDA <- read.csv('../data/process/tables/reduced_crc_model_top_vars_MDA_Summary.csv', header = T, stringsAsFactors = F)

# Create selection vector
common_vars <- common_vars_summary$otu

# Create MDA vectors
adn_MDA <- filter(adn_varsMDA, variable %in% common_vars) %>% slice(match(common_vars, variable)) %>% select(mean_MDA)
srn_MDA <- filter(srn_varsMDA, variable %in% common_vars) %>% slice(match(common_vars, variable)) %>% select(mean_MDA)
crc_MDA <- filter(crc_varsMDA, variable %in% common_vars) %>% slice(match(common_vars, variable)) %>% select(mean_MDA)

st2 <- select(common_vars_summary, otu) %>% 
  mutate(Tax = gsub("_unclassified", "", common_vars_summary$lowest_ID)) %>% 
  mutate(adn_MDA = adn_MDA$mean_MDA, srn_MDA = srn_MDA$mean_MDA, crc_MDA = crc_MDA$mean_MDA) %>% 
  arrange(desc(adn_MDA), desc(srn_MDA), desc(crc_MDA))

colnames(st2) <- c("OTU", "Lowest Taxonomic ID", "Adenoma Model MDA", "SRN Model MDA", "Carcinoma Model MDA")
...
adn_surg_summary <- read.csv('../data/process/tables/adn_combined_probs_surgery_pvalue_summary.csv', 
  ...
  mutate(variable = c("sobs", "Shannon", "Evenness", "Thetayc", "Adenoma Only\nPositive Probability", "SRN Only\nPositive Probablity"))
