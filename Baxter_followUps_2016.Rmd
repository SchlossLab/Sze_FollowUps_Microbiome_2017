...
```{r in_vs_fu, echo=FALSE, fig.height=6, fig.width=6}
init_vs_follow <- rbind(initial,followups)[,-1]
init_vs_follow$time <- factor(NA, levels=c(0,1))
init_vs_follow$time[1:67]<-0
init_vs_follow$time[68:134]<-1

pvals <- c()
for(i in 1:(ncol(init_vs_follow)-1)){
  pvals[i]<-aov(time~init_vs_follow[,i], data=init_vs_follow)$p.value
}




#rf_time <- AUCRF(time~., data=init_vs_follow, ntree=500, pdel=0.1, ranking='MDA')
