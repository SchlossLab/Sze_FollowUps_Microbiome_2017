...
```{r dists, echo=FALSE, fig.height=6, fig.width=6}
...
inter_ade <- dist[as.character(metaF[metaF$dx=='adenoma','initial']),as.character(metaF[metaF$dx=='adenoma','initial'])]
inter_ade <- as.numeric(as.vector(unlist(inter_ade)))

inter_canc <- dist[as.character(metaF[metaF$dx=='cancer','initial']),as.character(metaF[metaF$dx=='cancer','initial'])]
inter_canc <- as.numeric(as.vector(unlist(inter_canc)))

inter <- dist[as.character(metaF$initial),as.character(metaF$followUp)]
inter <- as.numeric(as.vector(unlist(inter)))


plot(density(inter), type='l',lwd=3, col='royalblue', ylim=c(0,4), xlab='thetaYC distance', main='', las=1, tck=-0.02)
#points(density(inter_ade), type='l', lwd=3, col='dark orange')
#points(density(inter_canc), type='l', lwd=3, col='dark red')
