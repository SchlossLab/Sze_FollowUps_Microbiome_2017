...
# Add extra columns for definitions to the rf summary table and remove spaces
...
for(i in 1:length(model_used)){
  ...
  for(j in 1:length(samples)){
    ...
    srn_compare_pvalue_summary[i, j] <- wilcox.test(
      filter(rf_prediction_summary, model == paste(model_used[i]), 
             sample_type == paste(samples[j]), Dx_Bin == "adv_adenoma")[, "postive_probability"], 
      filter(rf_prediction_summary, model == paste(model_used[i]), sample_type == paste(samples[j]), 
...
for(i in 1:length(model_used)){
  ...
  srn_compare_pvalue_summary[i, "change"] <- wilcox.test(
    filter(rf_prediction_summary, model == paste(model_used[i]), 
           sample_type == "initial", Dx_Bin == "adv_adenoma")[, "postive_probability"] - 
      filter(rf_prediction_summary, model == paste(model_used[i]), 
             sample_type == "followup", Dx_Bin == "adv_adenoma")[, "postive_probability"], 
    filter(rf_prediction_summary, model == paste(model_used[i]), sample_type == "initial", 
           Dx_Bin == "adenoma")[, "postive_probability"] - 
      filter(rf_prediction_summary, model == paste(model_used[i]), sample_type == "followup", 
             Dx_Bin == "adenoma")[, "postive_probability"])$p.value
}
...
for(i in 1:length(model_used)){
  ...
  srn_compare_pvalue_summary[i, "proportion"] <- 
    makeANDfish_2by2(select(rf_prediction_summary, -diagnosis) %>% 
                     rename(diagnosis = Dx_Bin), c("initial", "followup"), 
                   c("Yes", "No"), aucrf_model_cutoff, model = FALSE, 
                   model_sample_type = NULL, model_type = paste(model_used[i]), 
                   remove_sample = "cancer")
}
