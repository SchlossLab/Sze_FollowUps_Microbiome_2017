...
for(i in 1:n){
  ...
  load(paste("exploratory/adn_Reducedfeatures_RF_model_", i, ".RData", sep=""))
  
  if(i == 1){
    write.csv(eighty_twenty_splits, "data/process/tables/adn_reduced_test_data_splits.csv", 
              row.names = F)
    write.csv(test_data_imps, "data/process/tables/adn_reduced_test_tune_data.csv", 
              row.names = F)
  }
...
write.csv(
  ...
  "data/process/tables/adn_Reduced_ROC_model_summary.csv", row.names = F)


# Get Ranges of 100 10-fold 20 times CV data (worse, best)
actual_data <- read.csv("data/process/tables/adn_reduced_test_tune_data.csv", header = T)

data_splits <- read.csv("data/process/tables/adn_reduced_test_data_splits.csv", 
...
write.csv(test_roc_data, 
          "data/process/tables/adn_reduced_test_data_roc.csv", row.names = F)
...
write.csv(auc_data_table, "data/process/tables/adn_reduced_auc_summary.csv")
...
write.csv(adn_model_top_vars_MDA_full_data, 
          "data/process/tables/adn_reduced_lesion_model_top_vars_MDA.csv", row.names = F)
