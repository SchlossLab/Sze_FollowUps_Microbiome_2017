...
for(i in 1:n){
  ...
  if(i == 1){
    write.csv(eighty_twenty_splits, "data/process/tables/adn_test_data_splits.csv", 
      row.names = F)
    write.csv(test_data, "data/process/tables/adn_test_tune_data.csv", 
      row.names = F)
  }
...
write.csv(
  ...
  "data/process/tables/adn_ROC_model_summary.csv", row.names = F)
...
write.csv(MDA_vars_summary[order(MDA_vars_summary[, "mean_MDA"], decreasing = TRUE), ], 
          "data/process/tables/adn_lesion_model_top_vars_MDA_Summary.csv", row.names = F)
...
write.csv(lesion_model_top_vars_MDA_full_data, 
          "data/process/tables/adn_lesion_model_top_vars_MDA_full_data.csv", row.names = F)
...
actual_data <- read.csv("data/process/tables/adn_full_test_data.csv", header = T, row.names = 1)

data_splits <- read.csv("data/process/tables/adn_test_data_splits.csv", 
...
save.image("exploratory/adn_rocs.RData")
