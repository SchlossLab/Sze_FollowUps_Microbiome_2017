...
loadLibs(c("dplyr", "caret","scales", "randomForest", "pROC"))

# Read in necessary data frames
test_data <- read.csv("results/tables/common_test_tune_data.csv", header = TRUE)
split_data_results <- read.csv("results/tables/common_ROC_model_summary.csv", header = TRUE, stringsAsFactors = F)
test_data_roc <- read.csv("results/tables/common_test_data_roc.csv", header = TRUE, stringsAsFactors = F)
auc_data_table <- read.csv("results/tables/common_auc_summary.csv", header = TRUE, row.names = 1)
...
write.csv(cbind(lesion = as.character(test_data$lesion), full_model_probs_predictions), 
          "results/tables/common_lesion_test_data_probs_summary.csv", row.names = F)
...
write.csv(test_follow_up_data, "results/tables/common_pred_data.csv", row.names = F)

write.csv(probability_data_table, 
          "results/tables/common_follow_up_probability_summary.csv", row.names = F)
