...
for(i in 1:n){
  ...
  load(paste("exploratory/srn_RF_model_", i, ".RData", sep=""))
  
  if(i == 1){
    write.csv(eighty_twenty_splits, "data/process/tables/srn_test_data_splits.csv", 
      row.names = F)
    write.csv(test_data, "data/process/tables/srn_test_tune_data.csv", 
      row.names = F)
  }
...
write.csv(OTU_appearance_table, 
  "data/process/tables/srn_rf_wCV_imp_vars_summary.csv", row.names = F)
...
write.csv(MDA_vars_summary[order(MDA_vars_summary[, "mean_MDA"], decreasing = TRUE), ], 
          "data/process/tables/srn_lesion_model_top_vars_MDA_Summary.csv", row.names = F)
...
write.csv(lesion_model_top_vars_MDA_full_data, 
          "data/process/tables/srn_lesion_model_top_vars_MDA_full_data.csv", row.names = F)
...
actual_data <- read.csv("data/process/tables/srn_test_tune_data.csv", 
  header = T, stringsAsFactors = F)

data_splits <- read.csv("data/process/tables/srn_test_data_splits.csv", 
  header = T, stringsAsFactors = F)
...
write.csv(test_roc_data, 
  "data/process/tables/srn_test_data_roc.csv", row.names = F)
...
save.image("exploratory/srn_rocs.RData")
