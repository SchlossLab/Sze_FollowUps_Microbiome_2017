...
write.csv(OTU_appearance_table, 
  "data/process/tables/crc_rf_wCV_imp_vars_summary.csv", row.names = F)
...
write.csv(MDA_vars_summary[order(MDA_vars_summary[, "mean_MDA"], decreasing = TRUE), ], 
          "data/process/tables/crc_lesion_model_top_vars_MDA_Summary.csv", row.names = F)
...
write.csv(lesion_model_top_vars_MDA_full_data, 
          "data/process/tables/crc_lesion_model_top_vars_MDA_full_data.csv", row.names = F)
...
actual_data <- read.csv("data/process/tables/crc_full_test_data.csv", header = T, row.names = 1)

data_splits <- read.csv("data/process/tables/crc_test_data_splits.csv", 
...
save.image("exploratory/crc_rocs.RData")
