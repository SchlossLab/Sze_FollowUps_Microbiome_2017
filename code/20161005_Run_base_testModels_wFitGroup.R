...
write.csv(cutoffs, "results/tables/withFITGroup.cutoffs.csv", row.names = F)
...
# Get the prediction tables for each group using initial and follow up data for full data set (not selected)
...
write.csv(df_InitFollow_ALL, "results/tables/withFITGroup.models.datatable.csv", row.names = F)
...
ggsave(file = "results/figures/adn_predictions_withFitGroup.tiff", adn_graph, width=8, height = 8, dpi = 300)
...
ggsave(file = "results/figures/crc_predictions_withFitGroup.tiff", crc_graph, width=8, height = 8, dpi = 300)
