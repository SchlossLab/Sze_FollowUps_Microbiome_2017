...
# Load needed data
load("exploratory/adn_randomized_treatment_model.RData")
...
for(i in 1:n){
  ...
  if(i == 1){
    ...
    write.csv(test_data, "data/process/tables/adn_randomization_treatment_test_tune_data.csv", 
              row.names = F)
  }
...
write.csv(
  ...
  "data/process/tables/adn_randomization_treatment_ROC_model_summary.csv", row.names = F)
...
write.csv(test_roc_data, 
          "data/process/tables/adn_randomization_treatment_test_data_roc.csv", row.names = F)
...
write.csv(OTU_appearance_table, 
          "data/process/tables/adn_randomization_treatment_imp_vars_summary.csv", row.names = F)
...
write.csv(MDA_vars_summary[order(MDA_vars_summary[, "mean_MDA"], decreasing = TRUE), ], 
          "data/process/tables/adn_randomization_treatment_top_vars_MDA_Summary.csv", row.names = F)
...
write.csv(adn_model_top_vars_MDA_full_data, 
          "data/process/tables/adn_randomization_treatment_top_vars_MDA_full_data.csv", row.names = F)
