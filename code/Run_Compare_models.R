...
write.csv(OTU_appearance_table, "results/tables/if_rf_wCV_imp_vars_summary.csv", row.names = F)

rm(list = setdiff(ls(), c("OTU_appearance_table")))

# Read in data tables
rf_imp_vars_summary <- read.csv("results/tables/rf_wCV_imp_vars_summary.csv", stringsAsFactors = F, header = T) %>% 
  filter(Variable != "fit_result" & Variable != "BMI" & Variable != "Gender.mYes" & Variable != "Age")
...
good_metaf <- read.csv("results/tables/mod_metadata/good_metaf_final.csv", stringsAsFactors = F, header = T) %>% 
  mutate(lesion_follow = ifelse(Disease_Free == "n", 1, 0)) %>% filter(!is.na(fit_followUp))

mod_metaf <- read.csv("results/tables/follow_up_probability_summary.csv", stringsAsFactors = F, header = T) %>% 
...
shared <- as.data.frame(apply(select(shared, -Group), 2, function(x) x/subsample))

# Find common OTUs
common_vars <- filter(OTU_appearance_table, Variable %in% rf_imp_vars_summary$Variable)
...
shared <- shared[as.character(mod_metaf$sampleID), common_vars$Variable] %>% 
  mutate(Dx_Bin = mod_metaf$Dx_Bin, sampleType = mod_metaf$sampleType, EDRN = rep(good_metaf$EDRN, 2))
...
otu_pvalue <- as.data.frame(matrix(nrow = length(rownames(common_vars)), ncol = 1, 
                          dimnames = list(nrow = common_vars[, "Variable"], ncol = "Pvalue")))
...
pvalue_table <- cbind(otu = rownames(tax_df), tax_df, Pvalue = otu_pvalue, 
              BH_corr = p.adjust(otu_pvalue$Pvalue, method = "BH"))

write.csv(pvalue_table, "results/tables/pvalue_common_imp_vars.csv")

# Create Figure 5 graph of the one significant OTU

shared <- select(shared, contains(rownames(pvalue_table[pvalue_table$BH_corr < 0.05, ])), 
                 Dx_Bin, sampleType, EDRN)

init_follow_change <- ggplot(shared, aes(factor(sampleType, levels = c("initial", "followup")), Otu000012, group = factor(EDRN))) + 
  geom_line(aes(color = factor(Dx_Bin, levels = c("adenoma", "adv_adenoma", "cancer")))) + 
  geom_point(aes(color = factor(Dx_Bin, levels = c("adenoma", "adv_adenoma", "cancer")))) + 
  theme_bw() + ylab("Relative Abundance") + xlab("") + 
  ggtitle(paste(rownames(pvalue_table[pvalue_table$BH_corr < 0.05, ]), " (", 
                tax_df[rownames(pvalue_table[pvalue_table$BH_corr < 0.05, ]), "Genus"], ")", sep ="")) + 
  scale_colour_manual(
    name = "Lesion Type", 
  ...
  geom_hline(data = train_test_shared, 
             aes(yintercept = mean(train_test_shared[, rownames(pvalue_table[pvalue_table$BH_corr < 0.05, ])])), 
             linetype = 2) + 
  theme(legend.position=c(0.85,0.85), plot.margin = unit(c(1, 1, 1, 1), "lines"), 
        plot.title = element_text(size=20, face="bold", hjust = 0.5), legend.title = element_text(face="bold"))
