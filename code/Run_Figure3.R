...
grid.arrange(
  ...
  filter(lesion_model_roc, run != "middle_roc") %>% 
    ...
    geom_polygon(data = filter(reduced_lesion_model_roc, run != "full_roc"), 
                 aes(y = specificities), alpha = 0.25, fill = "red") + 
  ...
  ggplot(IF_model_roc, aes(x = sensitivities, y = specificities)) + 
    geom_polygon(alpha = 0.25) + 
    geom_line(aes(group = run), size = 1.25, color = "black", alpha = 0.5) + 
    geom_polygon(data = filter(reduced_IF_model_roc, run != "full_roc"), 
                 aes(y = specificities), alpha = 0.25, fill = "red") + 
