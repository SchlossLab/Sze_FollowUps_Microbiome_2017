...
# Load needed data
load("exploratory/crc_treatment_reduced_RF_model_Imp_OTU.RData")
...
for(i in 1:n){
  ...
  if(i == 1){
    ...
    write.csv(test_data, "data/process/tables/red_crc_treatment_test_tune_data.csv", 
              row.names = F)
  }
...
write.csv(
  ...
  "data/process/tables/reduced_crc_treatment_ROC_model_summary.csv", row.names = F)
...
write.csv(test_roc_data, 
          "data/process/tables/reduced_crc_treatment_test_data_roc.csv", row.names = F)
