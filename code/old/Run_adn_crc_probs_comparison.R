...
adn_wilcox_pvalue_summary <- getProb_PairedWilcox(
  ...
  rown = c("all_adenoma", "SRN_only"), 
  not_group = c("cancer", "cancer"), 
  extra_specifics = c("adenoma", "adv_adenoma"))
...
all_wilcox_summary <- as.data.frame(all_wilcox_summary) %>% 
  mutate(model_type = c("red_adn", "red_adn", "red_crc")) %>% 
  mutate(comparison = c("all_adenoma", "SRN_only", "carcinoma_only"))
...
model_summary_info <- 
  cbind(info = rownames(get_confusion_data(tempList[[1]], good_metaf, to_filter1 = "cancer", 
              ...
              samples_tested = c(rep("all_adn", 18), rep("SRN", 18), rep("crc", 18)), 
              model_type = c(rep("red_adn", 36), rep("red_crc", 18)))
...
confusion_counts_summary <- c()
  ...
  confusion_counts_summary <- as.data.frame(rbind(
    ...
    make_confusionTable(tempList[[2]], good_metaf, column = "Dx_Bin", to_filter1 = "adenoma", 
