### Test the first Random Forest with Fit as categorical variable (> or < 100)
...
rm(selected_train, sens_specif_table, data, corr_pvalue_ROC_table, orig_probs, impfactor_Data_List, orig_RF_run, 
   selected_probs, selected_RF_run, variableList, modelList, orig_rf_opt, orig_roc, selected_roc, selected_rf_opt, train, 
   confirmed_vars, rocNameList)
...
for(i in 1:length(fit_group_train)){
  
  set.seed(050416)
  orig_RF_run[[i]] <- AUCRF(as.formula(paste(colnames(fit_group_train[[i]])[1], " ~ .", sep = "")), 
                            data=fit_group_train[[i]], pdel=0.05, ntree=500, ranking='MDA')
  orig_rf_opt[[i]] <- orig_RF_run[[i]]$RFopt
  orig_probs[[i]] <- predict(orig_rf_opt[[i]], type = 'prob')[, 2]
  orig_roc[[i]] <- roc(fit_group_train[[i]][, 1] ~ orig_probs[[i]])
...
for(i in 1:length(fit_group_train)){
  ...
  selected_train <- select(fit_group_train[[i]], one_of(colnames(fit_group_train[[i]])[1], 
                                                        rownames(orig_rf_opt[[i]]$importance)))
  ...
  write.csv(confirmed_vars[[i]], paste("results/tables/", colnames(fit_group_train[[i]])[1], 
                                       "_RFGroup_confirmed_vars.csv", sep = ""), row.names = F)
  # Select specific variables from optimum RF
  test_set <- select(fit_group_train[[i]], one_of(colnames(fit_group_train[[i]])[1], confirmed_vars[[i]][, 'otus']))
  # Use the selected data set in AUCRF now
  set.seed(050416)
  selected_RF_run[[i]] <- AUCRF(as.formula(paste(colnames(fit_group_train[[i]])[1], " ~ .", sep = "")), 
...
write.csv(sens_specif_table, "results/tables/ROCCurve_sens_spec_RFGroup.csv")
...
ggsave(file = "results/figures/ROCCurve_withFitGroup.tiff", width=8, height = 8, dpi = 300)
