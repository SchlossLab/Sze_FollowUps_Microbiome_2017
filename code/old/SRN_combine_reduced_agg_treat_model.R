...
# Load needed data
load("exploratory/srn_treatment_reduced_RF_model_Imp_OTU.RData")
...
for(i in 1:n){
  ...
  if(i == 1){
    ...
    write.csv(test_data, "data/process/tables/red_srn_treatment_test_tune_data.csv", 
              row.names = F)
  }
...
write.csv(
  ...
  "data/process/tables/reduced_srn_treatment_ROC_model_summary.csv", row.names = F)
...
write.csv(test_roc_data, 
          "data/process/tables/reduced_srn_treatment_test_data_roc.csv", row.names = F)
...
MDA_vars_summary <- as.data.frame(t(top_vars_MDA_by_run)) %>% summarise_all(funs(median, IQR)) %>% 
...
write.csv(MDA_vars_summary, 
          "data/process/tables/reduced_srn_treatment_top_vars_MDA_Summary.csv", row.names = F)
...
write.csv(srn_model_top_vars_MDA_full_data, 
          "data/process/tables/reduced_srn_treatment_top_vars_MDA_full_data.csv", row.names = F)
