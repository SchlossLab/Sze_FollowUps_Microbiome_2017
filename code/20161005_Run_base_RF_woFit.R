### Run the first Random Forest without Fit
...
train <- list(cancer = inner_join(metaI, shared, by = c("sample" = "Group")) %>% 
                ...
                select(cancer, contains("Otu0")) %>% na.omit() %>% mutate(cancer = factor(cancer)), 
              lesion = inner_join(metaI, shared, by = c("sample" = "Group")) %>% 
                filter(!sample %in% good_metaf$initial) %>% 
                select(lesion, contains("Otu0")) %>% na.omit() %>% mutate(lesion = factor(lesion)), 
              SRNlesion = inner_join(metaI, shared, by = c("sample" = "Group")) %>% 
                filter(!sample %in% good_metaf$initial) %>% 
                select(SRNlesion, contains("Otu0")) %>% na.omit() %>% mutate(SRNlesion = factor(SRNlesion)), 
              threeway = inner_join(metaI, shared, by = c("sample" = "Group")) %>% 
                filter(!sample %in% good_metaf$initial) %>% 
                select(threeway, contains("Otu0")) %>% na.omit() %>% mutate(threeway = factor(threeway)))
...
for(i in 1:length(train)){
  ...
  write.csv(confirmed_vars[[i]], paste("results/tables/", colnames(train[[i]])[1], "_woFit_confirmed_vars.csv", sep = ""), 
            row.names = F)
...
write.csv(sens_specif_table, "results/tables/ROCCurve_noFit_sens_spec.csv")
...
save.image("exploratory/RFwoFit.RData")
