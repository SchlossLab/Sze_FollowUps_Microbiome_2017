...
dataList <- list(
  ...
  polyp_dist = pickDistanceValues(
    as.character(c(metaF$initial[metaF$Dx_Bin == "adenoma"], 
                   metaF$followUp[metaF$Dx_Bin == "adenoma"])), 
    as.character(c(metaF$initial[metaF$Dx_Bin == "adenoma"], 
                   metaF$followUp[metaF$Dx_Bin == "adenoma"])), 
    thetaCompTotal, withMeta = FALSE), 
  #choose only srn distances
  polyp_dist = pickDistanceValues(
    as.character(c(metaF$initial[metaF$Dx_Bin == "adv_adenoma"], 
                   metaF$followUp[metaF$Dx_Bin == "adv_adenoma"])), 
    as.character(c(metaF$initial[metaF$Dx_Bin == "adv_adenoma"], 
                   metaF$followUp[metaF$Dx_Bin == "adv_adenoma"])), 
    thetaCompTotal, withMeta = FALSE),
  #choose only crc distances
  crc_dist = pickDistanceValues(
    as.character(c(metaF$initial[metaF$Dx_Bin == "cancer"], 
                   metaF$followUp[metaF$Dx_Bin == "cancer"])), 
    as.character(c(metaF$initial[metaF$Dx_Bin == "cancer"], 
                   metaF$followUp[metaF$Dx_Bin == "cancer"])), 
    thetaCompTotal, withMeta = FALSE))
...
breakDown_samples <- list(
  ...
  adn = c(rep("initial", length(metaF$Dx_Bin[metaF$Dx_Bin == "adenoma"])), 
          rep("follow_up", length(metaF$Dx_Bin[metaF$Dx_Bin == "adenoma"]))), 
  #call vector for srn follow up samples
  srn = c(rep("initial", length(metaF$Dx_Bin[metaF$Dx_Bin == "adv_adenoma"])), 
          rep("follow_up", length(metaF$Dx_Bin[metaF$Dx_Bin == "adv_adenoma"]))),
  #call vector for crc follow up samples
  crc = c(rep("initial", length(metaF$Dx_Bin[metaF$Dx_Bin == "cancer"])), 
          rep("follow_up", length(metaF$Dx_Bin[metaF$Dx_Bin == "cancer"]))))

# Create table to store the relevant PERMANOVA, R2, and F.Model values
beta_diver_summary <- as.data.frame(
  matrix(nrow = 4, ncol = 3, dimnames = list(
  rows = c("All", "Adenoma", "SRN", "Carcinoma"), 
  cols = c("F_model", "R2", "PERMANOVA"))))

# Create list to store the relevant MDS data for graphs
thetayc.mds.list <- list(all = c(), adn = c(), srn = c(), crc = c())
...
write.csv(thetayc.mds.list[["srn"]], "data/process/tables/thetayc_srn_IF.csv", row.names = F)
