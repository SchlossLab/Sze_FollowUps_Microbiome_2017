...
for(i in 1:n){
  ...
  load(paste("exploratory/crc_Reducedfeatures_RF_model_", i, ".RData", sep=""))
  
  if(i == 1){
    write.csv(eighty_twenty_splits, "data/process/tables/crc_reduced_test_data_splits.csv", 
              row.names = F)
    write.csv(test_data_imps, "data/process/tables/crc_reduced_test_tune_data.csv", 
              row.names = F)
  }
...
write.csv(
  ...
  "data/process/tables/crc_Reduced_ROC_model_summary.csv", row.names = F)


# Get Ranges of 100 10-fold 20 times CV data (worse, best)
actual_data <- read.csv("data/process/tables/crc_reduced_test_tune_data.csv", header = T)

data_splits <- read.csv("data/process/tables/crc_reduced_test_data_splits.csv", 
